...
def filter_storage_final_project(storage):
    ...
    '''
    # renaming certain variables for clarity, and filtering out only relevant rows
    ...
    # building  new dataframe with only relevant rows
...
def get_storage():
    ...
    if  os.path.exists(url_library_txt):
        ...
        library = get_library_clean(True)
...
def check_for_csv_folder():
    ...
    if os.path.exists(folder_path) and os.path.isdir(folder_path):
        ...
        #  Removed print message for greater clarity of more important messages
    else:
...
def download_and_extract_zip(url, zip_file_name, csv_folder):
    ...
    if response.status_code == 200:
        ...
        print(f"Zip file downloaded successfully and saved as '{zip_file_name}'")

        with zipfile.ZipFile(zip_file_name, 'r') as zip_ref:
            zip_ref.extractall(csv_folder)
        print(f"Zip file contents were extracted to '{csv_folder}'")
    else:
...
def path_builder(file):
    '''
    Neccessary for accessing files once they are in csv_folder
    Parameters: file, the csv file nane
    Returns: the path from cwd to csv_folder to csv file name 
    '''
    current_working_directory = os.getcwd()
...
def download_driver(storage, variable, year, period):
    ...
    row_index = get_storage_row(storage, variable, year, period)
    csv_file_name = get_csv_from_row(storage, row_index)

    # checking for and building csv_folder if needed
    ...
    if os.path.exists(csv_file_path):
        ...
        return csv_file_name # return file name for interfacing with back end functions
